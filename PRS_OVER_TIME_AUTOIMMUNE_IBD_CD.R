...
GASTRO=data.frame(Trait=names2,P=-log10(as.numeric(as.vector(sapply(strsplit(pvaluesAll[,2],split="\""),`[[`,10)))),
...
thres=-log10(0.05/(2))
GASTRO$Trait <- factor(GASTRO$Trait, levels = rev(GASTRO$Trait[orden_pvals]))
colnames(GASTRO)[which(colnames(GASTRO)=="P")]="-log10(p)"
GASTRO$Significance=ifelse(GASTRO$`-log10(p)`<thres,"Non-sig","sig")
GASTRO$Significance_BeforeBA=ifelse(GASTRO$P_BeforeBA<thres,"Non-sig","sig")
GASTRO$Significance_AfterBA=ifelse(GASTRO$P_AfterBA<thres,"Non-sig","sig")
GASTRO=GASTRO[orden_pvals,]
GASTRO2=rbind(GASTRO,GASTRO,GASTRO)
GASTRO2[3:4,]$`-log10(p)`=GASTRO$P_BeforeBA
GASTRO2[3:4,]$beta=GASTRO$beta_BeforeBA
GASTRO2[3:4,]$Significance=GASTRO$Significance_BeforeBA
GASTRO2[5:6,]$`-log10(p)`=GASTRO$P_AfterBA
GASTRO2[5:6,]$beta=GASTRO$beta_AfterBA
GASTRO2[5:6,]$Significance=GASTRO$Significance_AfterBA
GASTRO2$Timing=NA
GASTRO2$Timing[1:2]="All"
GASTRO2$Timing[3:4]="Before_BA"
GASTRO2$Timing[5:6]="After_BA"
GASTRO2$P_BeforeBA=NULL
GASTRO2$P_AfterBA=NULL
GASTRO2$beta_BeforeBA=NULL
GASTRO2$beta_AfterBA=NULL
GASTRO2$Significance_BeforeBA=NULL
GASTRO2$Significance_AfterBA=NULL
GASTRO2$p=10^(-GASTRO2$`-log10(p)`)
